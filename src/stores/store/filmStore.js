    ...
    actions: {
        async queryManyFilmAsync(context, params) {
            if (params.status=="1"){
                ...
                context.commit("SET_FILM_PAGE_INFO", data.data.result);
            }else if(params.status=="3"){
                let data = await api.film.queryManyFilmApi(params);
                context.commit("SET_CLASSIC_FILM_LIST", data.data.result);
            }else {
                let data = await api.film.queryManyFilmApi(params);
                context.commit("SET_FILM_PAGE_INFO", data.data.result);
            }
        ...
        async queryFutureFilmAsync(context) {
            ...
            context.commit("SET_FUTURE_FILM_LIST", data.data.result)
        },
